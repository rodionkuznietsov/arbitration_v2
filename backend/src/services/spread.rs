...
use crate::{exchanges::{bybit_ws::BybitWebsocket, gate_rs::GateWebsocket}, models::{exchange::{ExchangeType, SharedSpreads, Spread}, line::{Line, TimeFrame}}, services::market_manager::ExchangeWebsocket, storage::line_storage::{add_new_line, get_last_spread_of_all_exhchange_pairs}};

pub struct ExchangeSpread {
    bybit: Arc<BybitWebsocket>,
    gate: Arc<GateWebsocket>,
    shared_spreads: Arc<SharedSpreads>,
    pub spread_tx: Arc<tokio::sync::broadcast::Sender<(String, String, OrderedFloat<f64>)>>,
    pool: sqlx::PgPool
}
...
impl ExchangeSpread {
    pub fn new(
        ...
        Self { 
            bybit: bybit, 
            gate: gate, 
            shared_spreads: shared_spreads,
            spread_tx: spread_tx.into(),
            pool,
        }
    }

    pub fn spawn_local_spread_engine(&self) {
        ...
        tokio::spawn({
            let spreads = self.shared_spreads.clone();
    ...
    pub fn calculate_spread_for_chart(&self) {        
        tokio::spawn({
            let shared_spreads = self.shared_spreads.clone();
            let spread_tx = self.spread_tx.clone();
    ...
    pub fn spawn_spread_db_wirter(
        ...
        let spread_tx = self.spread_tx.clone();
        let (new_line_tx, mut new_line_rx) = mpsc::channel::<Line>(100);
            ...
            async move {
                while let Some(line) = new_line_rx.recv().await {
                    match add_new_line(&pool, line.clone()).await {
                        ...
                        Err(e) => { eprintln!("[ExchangeSpread]: {e}") }
                    }
                }
            }
        });
